...
def scrape_company_people(linkedin_company_url: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "linkedin_cookies.pkl not found — LinkedIn scraping requires authentication.\n"
                "Run: python manage_cookies.py  to capture your session.\n"
                "Returning None so the pipeline continues with website data only."
            )
            driver.quit()
            return None
