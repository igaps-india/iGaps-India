...
def run_pipeline(
    ...
    if not linkedin_url:
        logger.info("  [1a] No --linkedin-url provided. Skipping search...")
    ...
    payload: dict[str, Any] = {
        ...
        "saved":              False,   # dry_run=True from microservice; no DB write
    }
...
def _log_insights_summary(label: str, insights: Optional[dict[str, Any]]) -> None:
    ...
    # format_profile_insights returns a flat dict with these top-level keys
    edu    = insights.get("education",          [])
    ach    = insights.get("early_achievements", [])
    work   = insights.get("work_experience",    [])
    skills = insights.get("domain_skills",      [])
    name   = insights.get("person_name",        "")

    logger.info(
        "  ↳ %s — person=%s | edu=%d item(s) | achievements=%d item(s) | "
        "work_exp=%d item(s) | skills=%d item(s)",
        label,
        name or "(unknown)",
        len(edu)    if isinstance(edu,    list) else 0,
        len(ach)    if isinstance(ach,    list) else 0,
        len(work)   if isinstance(work,   list) else 0,
